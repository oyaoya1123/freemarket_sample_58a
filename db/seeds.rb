...
['トートバッグ','エコバッグ','リュック/バックパック','ボストンバッグ','スポーツバッグ','ショルダーバッグ','クラッチバッグ','ポーチ/バニティ','ボディバッグ/ウェストバッグ','マザーズバッグ','メッセンジャーバッグ','ビジネスバッグ','旅行用バッグ/キャリーバッグ','ショップ袋','和装用バッグ','かごバッグ','その他'],
...
parent = Category2.create(name: 'レディース')
...
#メンズの子・孫カテゴリー配列
man_child_array=['トップス','ジャケット/アウター','パンツ','靴','バッグ','スーツ','帽子','アクセサリー','小物','時計','水着','レッグウェア','アンダーウェア','その他']
man_grandchild_array=[['Tシャツ/カットソー(半袖/袖なし)','Tシャツ/カットソー(七分/長袖)','シャツ','ポロシャツ','タンクトップ','ニット/セーター','パーカー','カーディガン','スウェット','ジャージ','ベスト','その他'],
...
parent = Category2.create(name: 'メンズ')
man_child_array.each_with_index do |child, i|
 child = parent.children.create(name: child)
 man_grandchild_array[i].each do |grandchild|
   child.children.create(name: grandchild)
 end
end
...
baby_grandchild_array=[['トップス','アウター','パンツ','スカート','ワンピース','ベビードレス','おくるみ','下着/肌着','パジャマ','ロンパース','その他'],
...
parent = Category2.create(name: 'ベビー・キッズ')
...
interior_grandchild_array=[['食器','調理器具','収納/キッチン雑貨','弁当用品','カトラリー(スプーン等)','テーブル用品','容器','エプロン','アルコールグッズ','浄水機','その他'],
...
parent = Category2.create(name: 'インテリア・住まい・小物')
...
book_grandchild_array=[['文学/小説','人文/社会','ノンフィクション/教養','地図/旅行ガイド','ビジネス/経済','健康/医学','コンピュータ/IT','趣味/スポーツ/実用','住まい/暮らし/子育て','アート/エンタメ','洋書','絵本','参考書','その他'],
...
parent = Category2.create(name: '本・音楽・ゲーム')
...
#おもちゃ・ホビー・グッズ
hoby_child_array=['おもちゃ','タレントグッズ','コミック/アニメグッズ','トレーディングカード','フィギュア','楽器/器材','コレクション','ミリタリー','美術品','アート用品','その他']
hoby_grandchild_array=[['キャラクターグッズ','ぬいぐるみ','小物/アクセサリー','模型/プラモデル','ミニカー','トイラジコン','プラモデル','ホビーラジコン','鉄道模型','その他'],
...
parent = Category2.create(name: 'おもちゃ・ホビー・グッズ')
hoby_child_array.each_with_index do |child, i|
 child = parent.children.create(name: child)
 hoby_grandchild_array[i].each do |grandchild|
   child.children.create(name: grandchild)
 end
end
...
cosme_grandchild_array=[['ファンデーション','化粧下地','コントロールカラー','BBクリーム','CCクリーム','コンシーラー','フェイスパウダー','トライアルセット/サンプル','その他'],
...
parent = Category2.create(name: 'コスメ・香水・美容')
...
#家電・スマホ・カメラ
audio_child_array=['スマートフォン/携帯電話','スマホアクセサリー','PC/タブレット','カメラ','テレビ/映像機器','オーディオ機器','美容/健康','冷暖房/空調','生活家電','その他']
audio_grandchild_array=[['スマートフォン本体','バッテリー/充電器','携帯電話本体','PHS本体','その他'],
...
parent = Category2.create(name: '家電・スマホ・カメラ')
audio_child_array.each_with_index do |child, i|
 child = parent.children.create(name: child)
 audio_grandchild_array[i].each do |grandchild|
   child.children.create(name: grandchild)
 end
end
...
sport_grandchild_array=[['クラブ','ウエア(男性用)','ウエア(女性用)','バッグ ','シューズ(男性用)','シューズ(女性用)','アクセサリー','その他'],
...
parent = Category2.create(name: 'スポーツ・レジャー')
...
handmade_grandchild_array=[['ピアス','イヤリング','ネックレス','ブレスレット','リング','チャーム','ヘアゴム','アンクレット','その他'],
...
parent = Category2.create(name: 'ハンドメイド')
...
ticket_grandchild_array=[['男性アイドル','女性アイドル','韓流','国内アーティスト','海外アーティスト','音楽フェス','声優/アニメ','その他'],
...
parent = Category2.create(name: 'チケット')
...
car_grandchild_array=[['国内自動車本体','外国自動車本体'],
...
parent = Category2.create(name: '自動車・オートバイ')
...
parent = Category2.create(name: 'その他')
